...
/*   Updated: 2025/12/13 04:42:03 by jdutille         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_dda_dist(t_data *data)
{
	...
	{
		ray->stepx = 1;
		ray->side_dx = (ray->mapx + 1 - data->map->posX) * ray->delta_dx;
	}
	else
	{
		ray->stepx = -1;
		ray->side_dx = (data->map->posX - ray->mapx) * ray->delta_dx;
	}
	if (ray->ray_dy > 0)
	{
		ray->stepy = 1;
		ray->side_dy = (ray->mapy + 1 - data->map->posY) * ray->delta_dy;
	}
	else
	{
		ray->stepy = -1;
		ray->side_dy = (data->map->posY - ray->mapy) * ray->delta_dy;
	}
}
...
void	init_dda(t_map *map, t_ray *ray, t_data *data)
{
	// printf("DEBUG: posX: %f, posY: %f\n", data->map->posX,
		// data->map->posY);
	ray->ray_dx = map->dirX + map->planeX * map->cameraX;
		// cos(ray->ray_angle);
	ray->ray_dy = map->dirY + map->planeY * map->cameraX;
		// sin(ray->ray_angle);
	// printf("DEBUG: map_x: %d, map_y: %d\n", ray->mapx, ray->mapy);
	ray->mapx = (int)(data->map->posX);
	ray->mapy = (int)(data->map->posY);
...
double	dda_dist(t_data *data, t_ray *ray, int ray_id)
{
	...
	// printf("SALUT\n");
	if (ray->side == 0)
		perp_dist = (ray->mapx - data->map->posX + (1 - ray->stepx) / 2)
			/ ray->ray_dx;
	else
		perp_dist = (ray->mapy - data->map->posY + (1 - ray->stepy) / 2)
			/ ray->ray_dy;
	if (ray->side == 0)
		wall_x = data->map->posY + perp_dist * ray->ray_dy;
	else
...
void	draw_wall(t_data *data, t_ray *ray, int ray_id)
{
	...
	y = 0;
	while (y < d_start)
		my_mlx_pixel_put(&data->img3d, ray_id, y++, 0x00FEFF); //plus qu'a set textures
	while (y <= d_end)
	{
	...
	while(y < HEIGHT)
		my_mlx_pixel_put(&data->img3d, ray_id, y++, 0xFF0000); //rgb(255,0,0)
